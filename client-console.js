...
// --- Button icons (global scope) ---
const SVG_BTN_EDIT = `<svg width="14" height="14" viewBox="0 0 32 32" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style="display:block">
...
const SVG_BTN_SAVE = `<svg width="14" height="14" viewBox="0 0 32 32" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style="display:block">
  <path d="M15.293,21.717l-7,-6.834a.943.943,0,0,1,0,-1.367l1.414,-1.368a1.029,1.029,0,0,1,1.414,0L16,16.934,26.879,6.283a1.025,1.025,0,0,1,1.414,0L29.707,7.65a.942.942,0,0,1,0,1.366l-13,12.7A1.025,1.025,0,0,1,15.293,21.717Z"/>
  <path d="M30.02,16h-.04A1.979,1.979,0,0,0,28,17.98V27a1,1,0,0,1-1,1H5a1,1,0,0,1-1,-1V5A1,1,0,0,1,5,4H22.02A1.979,1.979,0,0,0,24,2.02V1.98A1.979,1.979,0,0,0,22.02,0H2A2,2,0,0,0,0,2V30a2,2,0,0,0,2,2H30a2,2,0,0,0,2,-2V17.98A1.979,1.979,0,0,0,30.02,16Z"/>
</svg>`;
...
function outCollect_(){
 const { f } = outEls_();
const v = (x)=> (x && x.value ? String(x.value).trim() : "");
const join5 = (arr)=> Array.isArray(arr) ? arr.map(v).join("\n").replace(/\n+$/,"") : v(arr);
...
return {
  ...
  func: join5(f.func),
  goals: join5(f.goals),
  practical: join5(f.practical)
};
}

function outApplyEdit_(on){
  const { acc, btn, sp, msg, f } = outEls_();
  if (!acc) return;
...
const setRO = (arr)=>{
  if (!arr) return;
  (Array.isArray(arr) ? arr : [arr]).forEach(el => { if (el) el.readOnly = ro; });
};
setRO(f.area);
setRO(f.env);
setRO(f.func);
...
setRO(f.practical);
  ...
  if (msg){ msg.classList.add("hidden"); msg.textContent = ""; msg.classList.remove("is-err"); }
}

function outSetMsg_(text, isErr){
  const { msg } = outEls_();
  if (!msg) return;
  if (!text){
    msg.classList.add("hidden");
...
function outSetBaselineFromDom_(){
  try { OUT_BASELINE_STR = JSON.stringify(outCollect_()); }
  catch(_) { OUT_BASELINE_STR = ""; }
}

async function outSave_(){
  const { acc, btn, sp } = outEls_();
  if (!acc || !btn) return;

  // если залочено (sigLocked) — ничего не делаем
  const lockEl = acc.querySelector(".cc-sectionLock");
  if (lockEl && lockEl.classList.contains("is-locked")) return;

  // 1) не в edit-mode -> включаем редактирование
  if (acc.dataset.edit !== "1"){
    outApplyEdit_(true);
    return;
  }

  if (OUT_SAVING) return;
  ...
  try{
    ...
    const out = await api_("save_outputs", {
      client_id: st.client_id,
      session_token: st.session_token,
      out_json_b64: b64u_(curStr)
    });

    if (!out || !out.ok){
      outSetMsg_("Ошибка сохранения: " + ((out && out.error) || "unknown"), true);
      return;
    }

    OUT_BASELINE_STR = curStr;
    outApplyEdit_(false);
    outSetMsg_("Сохранено", false);
    setTimeout(()=>outSetMsg_("", false), 2500);
...
function outInitUi_(outputs){
  const acc = document.querySelector(".cc-exp-acc--outs");
  if (!acc || acc.dataset.bound === "1") return;
  acc.dataset.bound = "1";
...
put5(f.practical, o.practical);
  ...
  if (btn && btn.dataset.bound !== "1"){
    btn.dataset.bound = "1";
    btn.addEventListener("click", outSave_);
  }
}
...
function renderExperienceHtml_(exp, sigLocked){
  ...
  const outs = (exp && exp.outputs && typeof exp.outputs === "object") ? exp.outputs : {};
  const out5_ = (v)=>{
    if (Array.isArray(v)) {
  ...
  const outA = out5_(outs.area);
  const outE = out5_(outs.env);
  const outF = out5_(outs.func);
  const outG = out5_(outs.goals);
  ...
  const SVG_STAR = `<svg width="18" height="18" viewBox="0 0 64 64" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
<path d="M25,7h-6V6c0-1.7-1.3-3-3-3h-4c-1.7,0-3,1.3-3,3v1H3c-1.7,0-3,1.3-3,3v8c0,2.8,2.2,5,5,5h5.2c-0.1-0.3-0.2-0.6-0.2-1v-3c0-0.6,0.4-1,1-1h5c1.8-2.4,4.7-4,8-4c1.4,0,2.8,0.3,4,0.8V10C28,8.3,26.7,7,25,7z M17,7h-6V6c0-0.6,0.4-1,1-1h4c0.6,0,1,0.4,1,1V7z"/>
  ...
  return `
<details class="cc-exp-acc cc-exp-acc--prof">
  <summary class="cc-exp-accSum">
    <span class="cc-exp-ico cc-exp-ico-prof">${SVG_BRIEFCASE}</span>
    <div class="cc-exp-title cc-exp-accTitle">Профессиональный опыт</div>
...
<details class="cc-exp-acc cc-exp-acc--concl" style="margin-top:12px;">
  <summary class="cc-exp-accSum">
    <span class="cc-exp-ico cc-exp-ico-concl" aria-hidden="true">${SVG_CONCLUSION}</span>
...
<div id="conclMsg" class="cc-conclMsg hidden"></div>
    <div class="cc-conclTable">
      <div class="cc-conclRow cc-conclRow--head">
        ...
        <div class="cc-conclCell cc-conclCell--head">Объективные причины</div>
        <div class="cc-conclCell cc-conclCell--head">Субъективные / личные причины</div>
      </div>
...
<details class="cc-exp-acc cc-exp-acc--sig" style="margin-top:12px;">
  <summary class="cc-exp-accSum">
    <span class="cc-exp-ico cc-exp-ico-sig">${SVG_STAR}</span>
    <div class="cc-exp-title cc-exp-accTitle">Значимый опыт</div>
...
<details id="outsAcc" class="cc-exp-acc cc-exp-acc--outs" style="margin-top:12px;">
  <summary class="cc-exp-accSum">
    <span class="cc-exp-ico cc-exp-ico-outs" aria-hidden="true">${SVG_OUTS}</span>
    <div class="cc-exp-title cc-exp-accTitle">Выводы</div>
    ${sigLocked ? `<span class="cc-exp-accTag">Заблокировано</span>` : ``}
    <span class="cc-exp-accChev" aria-hidden="true">
      <svg viewBox="0 0 24 24"><path d="M6 9l6 6 6-6"/></svg>
    </span>
